...
from utils.helper import cert_gen
...
class OAuthRedirServer:        # pylint: disable=too-many-instance-attributes
    ...
    def run(self, on_success: Callable[[Any], None], on_failure: Callable[[str], None]):
        ...
        # Generate own self-signed cert for HTTPS
        if self.ssl_crt == None or self.ssl_key == None:
            crt_path, key_path = cert_gen()
        ...
        if self.__port_range:
            ...
            for port in ports:
                try:
                    self.__api_server = ApiServer('127.0.0.1', port, crt_path, key_path)
                    break
                except OSError:
                    pass
        else:
            self.__api_server = ApiServer('127.0.0.1', 0, crt_path, key_path)
        ...
        self.__thread = threading.Thread(target=self.__api_server.serve_forever,daemon=True)
        self.__thread.start()
